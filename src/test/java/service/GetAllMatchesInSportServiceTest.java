...
public class GetAllMatchesInSportServiceTest {
     ...
     * Making sure execute goes all the way through with normal input.
     */
    @Test
    public void testExecute_behaviour() {
        System.out.println("execute_behaviour");
        int id = 1;
        GetAllMatchesInSportService service = new GetAllMatchesInSportService();
        DbConn dbConn = mock(DbConn.class);
        Broker broker = getMockedBrokerFactoryWithBrokersSetup();
        service.init(dbConn, broker);
        service.execute(id);
        verify(broker.getMatchBroker(),times(1)).findAllSQL("SELECT * FROM matches WHERE league_id = ?",
                "0");
    }
       
    /**
     * Test of execute method, of class GetAllMatchesInSportService.
     *  Checking if null is returned when input id is invalid.
     */
    @Test
